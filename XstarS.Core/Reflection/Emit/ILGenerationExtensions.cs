        ...
        public static void EmitLdarg(this ILGenerator ilGen, int index)
        {
            ...
            switch (index)
            {
                ...
                case int when (byte)index == index:
                    ilGen.Emit(OpCodes.Ldarg_S, (byte)index); break;
                default:
                    ilGen.Emit(OpCodes.Ldarg, (short)index); break;
            }
        }

        /// <summary>
        /// 发出将计算堆栈上的值弹出并存储到指定索引处的参数的指令，并放到当前指令流中。
        /// </summary>
        /// <param name="ilGen">要发出指令的 <see cref="ILGenerator"/> 对象。</param>
        /// <param name="index">要存储计算堆栈上的值的参数的索引。</param>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="ilGen"/> 为 <see langword="null"/>。</exception>
        /// <exception cref="ArgumentOutOfRangeException">
        /// <paramref name="index"/> 超出 16 位无符号整数能表示的范围。</exception>
        public static void EmitStarg(this ILGenerator ilGen, int index)
        {
            if (ilGen is null)
            {
                throw new ArgumentNullException(nameof(ilGen));
            ...
            {
                ilGen.Emit(OpCodes.Starg, (short)index);
            }
        }

        /// <summary>
        /// 发出将指定索引处的参数的地址加载到计算堆栈上的指令，并放到当前指令流中。
        /// </summary>
        /// <param name="ilGen">要发出指令的 <see cref="ILGenerator"/> 对象。</param>
        /// <param name="index">要加载地址到计算堆栈的参数的索引。</param>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="ilGen"/> 为 <see langword="null"/>。</exception>
        /// <exception cref="ArgumentOutOfRangeException">
        /// <paramref name="index"/> 超出 16 位无符号整数能表示的范围。</exception>
        public static void EmitLdarga(this ILGenerator ilGen, int index)
        {
            if (ilGen is null)
            {
                throw new ArgumentNullException(nameof(ilGen));
            ...
            {
                ilGen.Emit(OpCodes.Ldarga, (short)index);
            }
        }
