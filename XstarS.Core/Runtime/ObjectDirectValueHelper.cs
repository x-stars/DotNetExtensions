namespace XstarS.Runtime
{
    ...
    /// 提供对象的直接值相关的帮助方法。
    /// </summary>
    public static class ObjectDirectValueHelper
    {
        /// <summary>
        /// 提供指定类型的相关常数值。
        /// </summary>
        /// <typeparam name="T">对象的类型。</typeparam>
        private static class Constants<T>
        {
            /// <summary>
            /// 表示当前类型的直接值以字节为单位的大小。
            /// </summary>
            internal static readonly int Size = Constants<T>.GetSize();
        ...
        public static int SizeOf<T>() => Constants<T>.Size;
        ...
        public static unsafe bool DirectValueEquals<T>(this T value, T other)
        {
        ...
        public static unsafe int GetDirectValueHashCode<T>(this T value)
        {
            var refValue = __makeref(value);
            var ptrValue = *(void**)&refValue;
            var size = ObjectDirectValueHelper.SizeOf<T>();
        ...
        public static unsafe byte[] DirectValueToByteArray<T>(this T value)
        {
            var refValue = __makeref(value);
