...
namespace System.Diagnostics
{
    ...
    internal static class DebuggerAttributes
    {
        ...
        internal static DebuggerAttributeInfo ValidateDebuggerTypeProxyProperties(
            Type type, Type[] genericTypeArguments, object obj)
        {
        ...
        private static Type GetProxyType(Type type, Type[] genericTypeArguments)
        {
            ...
            var attrs = Attribute.GetCustomAttributes(type, typeof(DebuggerTypeProxyAttribute));
            if (attrs.Length != 1)
            {
                throw new InvalidOperationException($"Expected one DebuggerTypeProxyAttribute on {type}.");
            }
            var attr = (DebuggerTypeProxyAttribute)attrs[0];
        ...
        internal static string ValidateDebuggerDisplayReferences(object obj)
        {
            ...
            var attrs = Attribute.GetCustomAttributes(objType, typeof(DebuggerDisplayAttribute));
            if (attrs.Length != 1)
            {
                throw new InvalidOperationException($"Expected one DebuggerDisplayAttribute on {objType}.");
            }
            var attr = (DebuggerDisplayAttribute)attrs[0];
