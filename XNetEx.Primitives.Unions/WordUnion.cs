...
public unsafe struct WordUnion : IEquatable<WordUnion>, ISerializable
{
    ...
    /// <summary>
    /// 将 <see cref="WordUnion"/> 结构的新实例初始化为指定只读字节跨度表示的值。
    /// </summary>
    /// <param name="bytes">包含值的只读字节跨度。</param>
    public WordUnion(ReadOnlySpan<byte> bytes) : this() { this.LoadFrom(bytes); }
#endif
    ...
    public void CopyTo(byte[] bytes, int offset = 0)
    {
        ...
        fixed (byte* pBytes = &bytes[offset]) { *(ushort*)pBytes = this.UInt16; }
    }
    ...
    {
        if (bytes is null) { throw new ArgumentNullException(nameof(bytes)); }
        fixed (byte* pBytes = &bytes[offset]) { this.UInt16 = *(ushort*)pBytes; }
    }
