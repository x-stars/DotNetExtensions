...
namespace XstarS.ComponentModel
{
    ...
    /// </summary>
    public class BindableBuilder : BindableBuilderBase<object>
    {
        /// <summary>
        /// 对所有属性设置数据绑定的
        ...
        /// </summary>
        private static readonly ConcurrentDictionary<Type, Lazy<BindableBuilder>>
            LazyBindableOnlyOfTypes = new ConcurrentDictionary<Type, Lazy<BindableBuilder>>();
        ...
        public static BindableBuilder Default(Type type) =>
            BindableBuilder.LazyDefaultOfTypes.GetOrAdd(type,
                newType => new Lazy<BindableBuilder>(
                    () => new BindableBuilder(newType, false))).Value;
        ...
        /// <exception cref="TypeAccessException">
        /// <paramref name="type"/> 不是公共接口，也不是公共非密封类。</exception>
        public static BindableBuilder BindableOnly(Type type) =>
            BindableBuilder.LazyBindableOnlyOfTypes.GetOrAdd(type,
                newType => new Lazy<BindableBuilder>(
                    () => new BindableBuilder(newType, true))).Value;
