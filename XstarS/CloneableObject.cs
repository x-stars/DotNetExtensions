...
namespace XstarS
{
    ...
    internal sealed class CloneableObject
    {
        ...
        public object DeepClone()
        {
            ...
            {
                this.CloneArrayElements((Array)clone);
            }
            // 其他类型，调用对应方法以创建字段的副本。
            else
            {
                this.CloneObjectMembers(clone);
            }
        ...
        private object GetCloneOf(object value)
        {
        ...
        /// 将数组的每个元素替换为其副本。
        /// </summary>
        /// <param name="array">要对每个元素创建副本的数组。</param>
        private void CloneArrayElements(Array array)
        {
            // 仅创建非指针元素的副本。
            if (!array.GetType().GetElementType().IsPointer)
            {
                ...
                if (array.Rank == 1)
                {
                    ...
                    {
                        array.SetValue(this.GetCloneOf(array.GetValue(i)), i);
                    }
                }
                ...
                else
                {
                    for (long i = 0; i < array.LongLength; i++)
                    {
                        ...
                        array.SetValue(this.GetCloneOf(array.GetValue(a)), a);
                    }
                }
            }
        }
        ...
        /// 将对象的每个实例字段替换为其副本。
        /// </summary>
        /// <param name="object">要对实例字段创建副本的对象。</param>
        private void CloneObjectMembers(object @object)
        {
            // 将每个非指针实例字段的值替换为其副本。
            var fields = @object.GetType().GetFields(BindingFlags.Instance |
                BindingFlags.Public | BindingFlags.NonPublic);
            foreach (var field in fields)
            {
                if (!field.FieldType.IsPointer)
                {
                    field.SetValue(@object, this.GetCloneOf(field.GetValue(@object)));
                }
            }
        }
    }
